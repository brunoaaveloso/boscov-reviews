...
const AdminFilmes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/filmes/admin`, {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      setFilmes((filmes) =>
        filmes.map((f) => (f.id === id ? { ...f, ativo: false } : f))
      );
  ...
  const handleRestore = async (id: number) => {
    try {
      ...
      setFilmes((filmes) =>
        filmes.map((f) => (f.id === id ? { ...f, ativo: true } : f))
      );
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setFilmes((filmes) =>
        filmes.map((f) => (f.id === editId ? { ...f, ...updated } : f))
      );
      setEditId(null);
      setEditData({});
          ...
          <tbody>
            {filmes.map((filme) => (
              <tr key={filme.id} className="bg-zinc-800">
                ...
                <td>
                  <span
                    className={filme.ativo ? "text-green-400" : "text-red-400"}
                  >
                ...
                <td className="space-x-2">
                  {editId === filme.id ? (
                    ...
                    <>
                      <Button size="sm" onClick={() => handleEdit(filme)}>
                        Editar
                      </Button>
                      {filme.ativo ? (
                        <Button
                        ...
                        <Button
                          size="sm"
                          variant="default"
                          onClick={() => handleRestore(filme.id)}
                        >
                          Restaurar
                        </Button>
                      )}
